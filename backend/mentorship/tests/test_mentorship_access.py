...
class MentorshipAccessTestCase(TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_mentor2_viewing_mentorship_involving_mentor1_mentee1(self):
        self.client.login(email=self.login_mentor2['email'], password=self.login_mentor2['password'])
        res = self.client.get(f'/api/mentorship/mentorship/{self.mentorship.uid}/')
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)

    def test_mentee2_viewing_mentorship_involving_mentor1_mentee1(self):
        self.client.login(email=self.login_mentee2['email'], password=self.login_mentee2['password'])
        res = self.client.get(f'/api/mentorship/mentorship/{self.mentorship.uid}/')
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)

    def test_mentor1_viewing_mentorship_involving_mentor1_mentee1(self):
        self.client.login(email=self.login_mentor1['email'], password=self.login_mentor1['password'])
        res = self.client.get(f'/api/mentorship/mentorship/{self.mentorship.uid}/')
        self.assertEqual(res.status_code, status.HTTP_200_OK)

    def test_mentee1_viewing_mentorship_involving_mentor1_mentee1(self):
        self.client.login(email=self.login_mentee1['email'], password=self.login_mentee1['password'])
        res = self.client.get(f'/api/mentorship/mentorship/{self.mentorship.uid}/')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
